...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['save'])) {
        ...
        $job_title = trim($_POST['Job title']); // Matches the column name with space
        $job_role = trim($_POST['dt_job_role']);
        $skillness = trim($_POST['skilness']);
        $date_joined = $_POST['date_joint'];
        $address = trim($_POST['address']);
        $contact_tel = trim($_POST['contact_tel']);
        $office_location = trim($_POST['office _location']); // Matches the column name with space
        ...
        try {
            ...
            $query = "INSERT INTO human_resource_list_dt (
                        ...
                        manpower_agent, employee_id, `Job title`, dt_job_role, skilness, date_joint, 
                        address, contact_tel, `office _location`, date_of_birth, per_hr_rate, sat_rate, sun_rate
                      ) VALUES (
                        :sr_no, :dt_employee_number, :dt_system_hr_number, :name, :calling_name, :category, 
                        :manpower_agent, :employee_id, :job_title, :dt_job_role, :skilness, :date_joint, 
                        :address, :contact_tel, :office_location, :date_of_birth, :per_hr_rate, :sat_rate, :sun_rate
                      )";
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="skilness" class="form-control">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label>Date Joined</label>
                                    <input type="date" name="date_joint" class="form-control">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label>Address</label>
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $.ajax({
                    ...
                    success: function(employee) {
                        ...
                        $('[name="Job title"]').val(employee['Job title']);
                        $('[name="dt_job_role"]').val(employee.dt_job_role);
                        $('[name="skilness"]').val(employee.skilness);
                        $('[name="date_joint"]').val(employee.date_joint);
                        $('[name="address"]').val(employee.address);
                        $('[name="contact_tel"]').val(employee.contact_tel);
                        $('[name="office _location"]').val(employee['office _location']);
                ...
                $.ajax({
                    ...
                    success: function(employee) {
                        ...
                        var details = `
                            ...
                            <p><strong>Skillness:</strong> ${employee.skilness}</p>
                            <p><strong>Date Joined:</strong> ${employee.date_joint}</p>
                            <p><strong>Address:</strong> ${employee.address}</p>
                            <p><strong>Contact Tel:</strong> ${employee.contact_tel}</p>
                            <p><strong>Office Location:</strong> ${employee['office _location']}</p>
